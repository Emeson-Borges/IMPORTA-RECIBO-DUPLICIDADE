...
def buscar_informacoes_por_cpf(caminho_arquivo, cpf, namespace_evt, namespace_retorno):
    try:
        ...
        # Encontrar o CPF no XML
        cpf_xml = root.find(f".//{ns('ideBenef', namespace_evt)}/{ns('cpfBenef', namespace_evt)}")

        if cpf_xml is not None and cpf_xml.text == cpf:
            # Encontrar o ID do evento
            id_evento_novo = root.find(f".//{ns('evtPgtos', namespace_evt)}").attrib.get("Id")
...
def salvar_resultados_em_txt(resultados, caminho_arquivo_txt):
    with open(caminho_arquivo_txt, "w") as f:
        for resultado in resultados:
            f.write(
                f"update esocial.s1210 set idevento='{resultado[0]}', situacao='1' where cpfbenef='{resultado[2]}' and idevento='{resultado[3]}' and perapur='2024-12';\n\n"
            )
            f.write(
...
# Namespaces do XML
namespace_evt = "http://www.esocial.gov.br/schema/evt/evtPgtos/v_S_01_03_00"
...
caminho_pasta_xml = "C:/Users/itarg/Downloads/xml-PMAnapu-Separado/S-1210"
...
caminho_arquivo_txt = "C:/Users/itarg/Downloads/SCRIPTS-UPDATE-S1210.txt"
