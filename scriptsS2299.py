...
def buscar_informacoes_por_cpf(caminho_arquivo, cpf, namespace_main, namespace_evt, namespace_retorno):
    try:
        ...
        evento = root.find(f".//{ns('retornoProcessamentoDownload', namespace_main)}/{ns('evento', namespace_main)}/{ns('eSocial', namespace_evt)}/{ns('evtDeslig', namespace_evt)}")
...
def salvar_resultados_em_txt(resultados, caminho_arquivo_txt):
    with open(caminho_arquivo_txt, "w") as f:
        for resultado in resultados:
            f.write(
                f"update esocial.s2299 set idevento='{resultado[0]}', situacao='1' where cpftrab='{resultado[2]}' and idevento='{resultado[3]}';\n\n"
            )
            f.write(
                f"update esocial.historico set idevento='{resultado[0]}', nr_recibo='{resultado[1]}', message='201 - Lote processado com sucesso', status='P' where idevento='{resultado[3]}' AND evento='S2299';\n\n"
            )

# Namespaces do XML
namespace_main = "http://www.esocial.gov.br/schema/download/retornoProcessamento/v1_0_0"
namespace_evt = "http://www.esocial.gov.br/schema/evt/evtDeslig/v_S_01_03_00"
...
caminho_pasta_xml = "C:/Users/itarg/Downloads/xml-PMAnapu-Separado/S-2299"
...
caminho_arquivo_txt = "C:/Users/itarg/Downloads/SCRIPT-SQL2299-PMANAPU-final.txt"
